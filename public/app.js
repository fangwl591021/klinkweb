  ...
  $("#startProductAsk").onclick = async () => {
    ...
    try {
      ...
      const productCards = products.map((product) => {
        ...
        const status = product.reviewStatus ? '<small class="smart-product-review-status">審核狀態：' + esc(product.reviewStatus) + '</small>' : "";
        const facts = product.approvedPublicFacts?.length ? product.approvedPublicFacts.join(" ") : (product.facts || "");
        return '<article class="smart-product-item"><div><strong>' + esc(product.productName || product.name || "") + '</strong><small>' + esc([product.productSeries || product.plan, product.kind, product.code].filter(Boolean).join("・")) + '</small>' + status + '</div>' + (!pending && product.size ? '<b>' + esc(product.size) + '</b>' : '') + '<p>' + esc(facts) + '</p>' + (!pending && product.ingredients ? '<details><summary>成分／材質</summary><p>' + esc(product.ingredients) + '</p></details>' : '') + (!pending && product.usage ? '<details><summary>使用方式</summary><p>' + esc(product.usage) + '</p></details>' : '') + '</article>';
      }).join("");
      const productAnswer = '<section class="card smart-product-answer ' + (result.blocked ? "blocked" : "") + '"><div class="smart-product-answer-head"><h2>' + (result.blocked ? "此問題無法提供商品建議" : "商品顧問回覆") + '</h2>' + (result.quadrantLabel ? '<span>' + esc(result.quadrantLabel) + '說明</span>' : '') + '</div><p>' + esc(result.answer || "目前沒有可提供的商品資料。") + '</p>' + productCards + (result.disclaimer ? '<small class="smart-product-disclaimer">' + esc(result.disclaimer) + '</small>' : '') + (actions.length ? '<div class="smart-product-actions">' + actions.map((action) => '<a class="btn ' + (action.type === "line" ? "" : "alt") + '" href="' + esc(action.url) + '" target="_blank" rel="noopener">' + esc(action.label || "查看資料") + '</a>').join("") + '</div>' : '') + '</section>';
