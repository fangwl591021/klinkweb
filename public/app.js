  ...
  $("#startProductAsk").onclick = async () => {
    ...
    try {
      ...
      const productCards = visibleProducts.map((product) => {
        ...
        if (pending) return '<article class="smart-product-item"><header><strong>' + esc(product.productName || product.name || "") + '</strong></header></article>';
