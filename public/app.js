...
function openOfficialTall(page){const url=officialLiffUrl(page);try{if(window.liff?.isInClient?.()){window.liff.openWindow({url,external:false});return}}catch(error){console.warn("Official Tall LIFF window failed",error)}window.location.href=url}
function bindOfficialTallLinks(){const pages=["about","news","products","video"];document.querySelectorAll(".klink-home-grid>a").forEach((link,index)=>link.addEventListener("click",(event)=>{event.preventDefault();openOfficialTall(pages[index]||"home")}))}
function bindPortalActions(){document.querySelectorAll("[data-home-action]").forEach((button)=>(button.onclick=async()=>{const action=button.dataset.homeAction;if(action==="share")return showShareQr();if(action==="aiWear")return openAiWear();if(action==="walletqr"){const panel=$("#walletPanel");if(!panel){state.tab="wallet";return render()}$(".site-home-frame")?.classList.add("hidden");panel.classList.remove("hidden");panel.scrollIntoView({behavior:"smooth",block:"start"});return showWalletQr("homeWalletQr","homeWalletExpire")}state.tab=action==="home"?"home":action==="daily"?"daily":action==="courses"?"courses":action==="profile"?"profile":action==="card"?"card":action==="zodiac"?"zodiac":action==="cardCollection"?"cardCollection":"wallet";await render()}));bindOfficialTallLinks();$("#copyInvite")?.addEventListener("click",copyInvite)}
